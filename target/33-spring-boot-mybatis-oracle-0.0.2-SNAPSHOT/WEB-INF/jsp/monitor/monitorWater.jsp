...
<script type="text/javascript">
	...
	function add(){
		mini.open({
            url: "<%=basePath%>monitor/monitorAddAndEditWater",
            title: "新增测试计划井号", 
            width: 800, 
            height:550,
            onload: function () {
	...
	function openUploadFileDialog(){
		mini.open({
            url:"<%=basePath%>monitor/monitorExportWater",
            title: "水井注数据管理批量导入", 
            width: 550, 
            height: 350,
            onload: function () {
	...
	function submitData(){
		...
		$.ajax({
			type:"get",
			url:"<%=basePath%>waterjcjh/submitWaterJCJH?seqlist="+seqs,
			dataType:"text",
			success:function(data){
			 	var grid = mini.get("datagrid");
        		grid.reload(); 
	...
	function revocationData(){
		...
		$.ajax({
			type:"get",
			url:"<%=basePath%>waterjcjh/unSubmitOilJCJH?seqlist="+seqs,
			dataType:"text",
			success:function(data){
			 	var grid = mini.get("datagrid");
        		grid.reload(); 
	...
	<div id="div1" style="width: 100%;height: 100%">
		<div class="mini-toolbar" id="toolbar1">
			...
			<input id="dateType" class="mini-combobox" textField="text" valueField="id"  value="tc_date"  emptyText="日期类型" style="width:120px;" allowinput="true" shownullitem="false"  showClose="true" oncloseclick="onCloseClick" />
				<span class="separator"></span> 
			<input id="tj_status" class="mini-combobox" textField="text" valueField="id" showClose="false"  style="width:60px;" allowinput="false" shownullitem="false" emptyText="全选"/>
