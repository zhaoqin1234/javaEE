...
<script type="text/javascript">
	 ...
	 var jsonData2 = [{ 'id': '1', 'text': '已审核'},{ 'id': '0', 'text': '未审核'}];
	  mini.get("dateType").load(jsonData);
	  mini.get("sh_status").load(jsonData2);
		})
	//新增
	function add(){
		mini.open({
            ...
            onload: function () {
               ...
               var data = {data:{action: "new",dataType:"water"}};
		...
		function edit(){
              ...
              if(rowObj.length>1||rowObj.length<1){
              	mini.alert("请重新选择");
              	return ;
              }
            	for(var i=0;i<rowObj.length;i++){
        			if(rowObj[0].sh_status=="0"){
        				mini.alert("包含未审核出单内容，无法编辑！");
        				return ;
        			}
        			
        		}
			mini.open({
	            url: "<%=basePath%>monitor/monitorAddAndEditWaterEnd",
	            title: "编辑测试计划井号", 
	            width:  800, 
	            height: 550,
	            onload: function () {
	...
	/* 	if(dateType == ""||dateType==""){
		...
		var sh_status = mini.get("sh_status").getValue();  //提交状态
		var grid = mini.get("datagrid");
		var pIndex = grid.pageIndex;
		var pSize = grid.pageSize;
		grid.load({site_id:siteId,well_id:wellId,cs_type_id:cs_type_id,startDate:startDate,endDate:endDate,dateType:dateType,sh_status:sh_status,pIndex:pIndex,pSize:pSize});
	}
	...
	function remove(){
		...
		$.ajax({
			type:"get",
			url:"<%=basePath%>waterjcjh/delOneWaterJCJH?seq="+seqs,
			dataType:"text",
	...
	function openUploadFileDialog(){
		mini.open({
            url:"<%=basePath%>monitor/monitorExport",
            title: "月配注数据管理批量导入", 
            width: 550, 
            height: 350,
            onload: function () {
	...
	function submitData(){
		...
		var seqs=[];
		for(var i=0;i<rowsObj.length;i++){
			if(rowsObj[0].sh_status=="0"){
				mini.alert("包含未提交内容，无法进行审核！");
				return ;
			}
		...
		$.ajax({
			type:"get",
			url:"<%=basePath%>waterjcjh/auditWaterJCJH?seqlist="+seqs,
			dataType:"text",
			success:function(data){
			 	var grid = mini.get("datagrid");
        		grid.reload(); 
	...
	function revocationData(){
		...
		$.ajax({
			type:"get",
			url:"<%=basePath%>waterjcjh/unAuditWaterJCJH?seqlist="+seqs,
			dataType:"text",
			success:function(data){
			 	var grid = mini.get("datagrid");
        		grid.reload(); 
	...
	var data={site_id:siteId,well_id:wellId,cs_type_id:cs_type_id,startDate:startDate,endDate:endDate,dateType:dateType,sh_status:sh_status,pIndex:pIndex,pSize:pSize};
		 DownLoadExcel("<%=basePath%>waterjcjh/dwnloadQueryData?number="+Math.random(), data, false);
	}
...
<body style="width: 98%;height: 97%">
	<div id="div1" style="width: 100%;height: 100%">
		<div class="mini-toolbar" id="toolbar1">
			<span>工区:</span>
           	<input id="site_id" class="mini-combobox" textField="name" valueField="id"  showClose="true" oncloseclick="onCloseClick" onvaluechanged="getWellList()"  value="HBn4JTnVa2" 
			...
			<input id="well_id" class="mini-combobox" textField="wellCommonName" valueField="wellId"  showClose="true" oncloseclick="onCloseClick"
				...
				<span class="separator"></span> 
			<span>测试类型:</span>
			<input id="cs_type_id" class="mini-combobox" textField="jcjh_name" valueField="jcjh_id"  showClose="true" oncloseclick="onCloseClick" url="<%=basePath%>jcjhOption/getJCJHOptionWater"
			...
			<input name="endDate" type="text" id="endDate" style="width:120px;" class="mini-datepicker" format="yyyy-MM-dd" allowinput="false" emptyText="请选择..."   showClose="true" oncloseclick="onCloseClick"/>
				<span class="separator"></span> 
			<input id="dateType" class="mini-combobox" textField="text" valueField="id"  value="tc_date" emptyText="日期类型" style="width:120px;" allowinput="true" shownullitem="false"  showClose="true" oncloseclick="onCloseClick"/>
				<span class="separator"></span> 
			<input id="sh_status" class="mini-combobox" textField="text" valueField="id" showClose="false"  style="width:100px;" allowinput="false" shownullitem="false" emptyText="全选" />
			<a class="mini-button" iconCls="icon-search" onclick="search()">查询</a>
			     <a class="mini-button" iconCls="icon-edit" onclick="edit()">编辑</a>
			<a class="mini-button" iconCls="icon-download" onclick="exportData()">数据导出</a>
			<a class="mini-button" iconCls="icon-ok" onclick="submitData()">批量审核</a> 
			<a class="mini-button" iconCls="icon-no" onclick="revocationData()">批量撤销</a> 
		</div>
		<div id="datagrid" class="mini-datagrid" style="width:100%;height:92%;" url="<%=basePath%>waterjcjh/getWaterJCJHList" sizeList="[10,20,30,40,50,100,200,500,1000]"  ondrawcell="ondrawcell" allowCellWrap="true" enableHotTrack="true"
			idField="id" allowResize="true" sortMode="client" multiSelect="true" showReloadButton="false" pageSize="20" showPager="true" allowUnselect="true" frozenStartColumn="0" frozenEndColumn="3">
...
<!--           		<div field="cs_explaim" headerAlign="center" align="center" headerStyle="font-weight: bold;">测试内容</div> -->
          		...
          		<div field="cs_water_injection" headerAlign="center" align="center"  headerStyle="font-weight: bold;">测试要求日注水量</div>
