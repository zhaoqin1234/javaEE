...
<body style="width: 98%;height: 97%">
	<div id="div1">
		<div class="mini-toolbar" id="toolbar1">
			<span>工区:</span>
			<div id="site_id" class="mini-combobox" style="width:130px;"  textField="site_name" valueField="site_id" value="HBn4JTnVa2" 
			...
			<span class="separator"></span> 
			<a class="mini-button" iconCls="icon-download" onclick="editMarkMsg()">编辑备注信息</a>
		</div>
		
		<div class="mini-toolbar" id="toolbar1" headerAlign="center">
			<span align="center"><h2 id="bbName">监测计划报表</h2></span>
			<div id="out_msg_id">没有备注信息</div>
		</div>
...
<script type="text/javascript">
	...
	document.getElementById("div1").style.height = HeightEx - 130 + "px";
...
function DrugDataGridIntAnto (data) {
		...
		if(data != null ){
			...
			$("#out_msg_id").text(data.o_out_mes); 
			note_msg = data.o_out_mes;
			msg_note_id = data.o_out_note_id;
	...
	//编辑报表的备注信息 
	function editMarkMsg(){
		var date = mini.get("date").text;
		var csjh_type = mini.get("csjh_type").value;
		
		var csjh_typeText = mini.get("csjh_type").text;  //当前的
		if(msg_note_id == "" || msg_note_id == null ){
			mini.open({
                url:"<%=basePath%>measure/keyMeasuresPlanMark",
                title: "新增报表备注信息", 
                width: 550, 
                height: 380,
                onload: function () {
                    ...
                    var data = { action: "add",
                    			msg_note_id:msg_note_id,
                    			msg_note:note_msg,
                    			csjh_type:csjh_type,
                    			csjh_typeText:csjh_typeText,
                    			stime:date
                    			};
