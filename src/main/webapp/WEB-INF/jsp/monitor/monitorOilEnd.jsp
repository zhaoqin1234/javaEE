...
<script type="text/javascript">
	...
	function add(){
		...
		function edit(){
			...
			mini.open({
	            url: "<%=basePath%>monitor/monitorAddAndEditEnd",
	            title: "编辑测试计划井号", 
	            width:  800, 
	            height: 550,
	            onload: function () {
	...
	/* 	if(dateType == ""||dateType==""){
		...
		var sh_status = mini.get("sh_status").getValue();  //审核状态
		var grid = mini.get("datagrid");
	...
	function submitData(){
		...
		$.ajax({
			type:"get",
			url:"<%=basePath%>oiljcjh/auditOilJCJH?seqlist="+seqs,
			dataType:"text",
			success:function(data){
			 	var grid = mini.get("datagrid");
        		grid.reload(); 
	...
	function revocationData(){
		...
		$.ajax({
			type:"get",
			url:"<%=basePath%>oiljcjh/unAuditOilJCJH?seqlist="+seqs,
			dataType:"text",
			success:function(data){
			 	var grid = mini.get("datagrid");
        		grid.reload(); 
...
<body style="width: 98%;height: 97%">
	<div id="div1" style="width: 100%;height: 100%">
		<div class="mini-toolbar" id="toolbar1">
			...
			<span class="separator"></span> 
			<input id="dateType" class="mini-combobox" textField="text" valueField="id"  showClose="true" oncloseclick="onCloseClick" value="tc_date"  emptyText="日期类型" style="width:120px;" allowinput="true" shownullitem="false" />
			<span class="separator"></span> 
			<input id="sh_status" class="mini-combobox" textField="text" valueField="id" showClose="false"  style="width:65px;" allowinput="false" shownullitem="false" emptyText="全选"/>
			<span class="separator"></span> 
	...
	<!-- 	 <div style="float:right"> 
		...
		<div id="datagrid" class="mini-datagrid" style="width:100%;height:92%;" url="<%=basePath%>oiljcjh/getOilJCJHList" sizeList="[10,20,30,40,50,100,200,500,1000]" ondrawcell="ondrawcell" allowCellWrap="true" enableHotTrack="true"
