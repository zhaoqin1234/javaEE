...
public class MonthPZServiceImpl implements MonthPZService {
	...
	public List<String> batchInsertOrUpdate(List<MonthPZ> monthPZlist){
		...
		for (MonthPZ monthPZ : monthPZlist) {
			String result = null; //TODO insertOrUpdate(monthPZ);
			if(result != null) {
				list.add(result);
			}else {
	...
	public List<Dzs_pz_report_summary> getAllDzspzreportsummary() {
		...
		List<Dzs_pz_report_summary> selectAllDZS_PZ_REPORT_SUMMARY = monthPZMapper.selectAllDZS_PZ_REPORT_SUMMARY();
		
		
		return selectAllDZS_PZ_REPORT_SUMMARY;
	}
}
